...
export const createMaterialAnalysisBodySchema = Joi.object({
	...
	analysisId: Joi.array().items(Joi.string().required()).min(1).required(), // array of strings
});

export const updateMaterialAnalysisBodySchema = Joi.object({
	materialId: Joi.string().optional(),
	analysisId: Joi.array().items(Joi.string().required()).min(1).optional(), // array of strings
	status: Joi.string().valid("active", "inActive").optional(),
}).min(1);
